...
export default function Profile() {
  ...
  const handleArchivePost = async (postId: string) => {
    try {
      await postService.toggleArchivePost(postId)
      setPosts(prev => prev.filter(post => post._id !== postId))
      // Note: Archived posts are hidden from the profile but not deleted
    } catch (error) {
              ...
              {reels.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {reels.map((reel) => (
                    <div
                      key={reel._id}
                      className="aspect-[9/16] bg-black rounded-lg overflow-hidden relative cursor-pointer hover:opacity-90 transition-opacity"
