...
export default function ChatInterface({ conversation, onClose }: ChatInterfaceProps) {
      ...
      messagesPollInterval = setInterval(async () => {
        if (!socketService.isSocketConnected() && !loading) {
          try {
            ...
            const latestMessages = Array.isArray(messagesData) ? messagesData : []
  ...
  const loadMessages = async (page = 1) => {
    try {
      ...
      if (page === 1) {
        ...
        setMessages(prev => [[...newMessages].reverse(), ...prev].flat())
      }

      setHasMoreMessages(response.data?.pagination?.hasNext || response.pagination?.hasNext || false)
  ...
  const handleSendMessage = async (content: string, media?: File, replyTo?: Message) => {
    ...
    try {
      ...
      // Note: The backend messageController already emits the message to other users
      // No need to emit via socket again
