...
class SocketService {
  ...
  connect() {
    ...
    console.log('🔌 Attempting Socket.IO connection with authentication token')
  ...
  private setupEventListeners() {
    ...
    this.socket.on('connect_error', (error) => {
        ...
        if (authStore.tokens?.refreshToken) {
          ...
          authStore.refreshToken().then(success => {
            if (success) {
              ...
              console.error('❌ Token refresh failed. User may need to login again.')
            }
          })
          return // Don't try normal reconnection
        }
      }
      ...
      if (import.meta.env.DEV) {
        ...
        console.warn('💡 Verify authentication token is valid')
      }
  ...
  // Note: Message sending is handled via API calls, not socket events
  // Real-time message updates are received via 'new_message' events from the server

  startTyping(conversationId: string) {
    if (this.socket && this.isConnected) {
      console.log(`⌨️  Started typing in conversation: ${conversationId}`)
