...
// Get user followers
export const getUserFollowers = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { userId } = req.params;
  const currentUserId = req.user?._id;
  const page = parseInt(req.query.page as string) || 1;
  const limit = parseInt(req.query.limit as string) || 20;
  ...
  // Check if current user can view followers (public account or following)
  const canViewFollowers = 
    currentUserId === userId || // Own followers
    !targetUser.profile.isPrivate || // Public account
    (currentUserId && targetUser.followers.includes(currentUserId as any)); // Following private account
  ...
  const followersWithStatus = await Promise.all(
    followers.map(async (follower) => {
      ...
      if (currentUserId && currentUserId !== follower._id.toString()) {
        const currentUser = await User.findById(currentUserId);
  ...
  const totalFollowers = targetUser.followers.length;
...
// Get user following
export const getUserFollowing = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { userId } = req.params;
  const currentUserId = req.user?._id;
  const page = parseInt(req.query.page as string) || 1;
  const limit = parseInt(req.query.limit as string) || 20;
  ...
  // Check if current user can view following (public account or following)
  const canViewFollowing = 
    currentUserId === userId || // Own following
    !targetUser.profile.isPrivate || // Public account
    (currentUserId && targetUser.followers.includes(currentUserId as any)); // Following private account
  ...
  const followingWithStatus = await Promise.all(
    following.map(async (followingUser) => {
      ...
      if (currentUserId && currentUserId !== followingUser._id.toString()) {
        const currentUser = await User.findById(currentUserId);
  ...
  const totalFollowing = targetUser.following.length;
